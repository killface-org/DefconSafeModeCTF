...
const JWT_KEY = '011FFAVm4uKhI02s48aXqqQw4BhCwHnM';
...
{
    const token = req.cookies['lvl2token'];
    if (!token)
    {
        return false;
    }
...
router.all('/logout', async(req, res) => {
    res.clearCookie('lvl2token', { path: '/level2' });
    res.redirect('./');
});
...
router.post('/userinfo', cookieParser(), async(req, res) => {
    if (await isUserAuthenticated(req))
    {
        const verify = jwt.verify(req.cookies['lvl2token'], JWT_KEY);
        res.json(verify);
    }
    else
    {
...
async function publishPost(userID, postData, isPublic)
{
    ...
    {
        console.log('Level 2: publishPost');
        console.log(err);
        return {
            status: 'failed',
            message: err.toString()
...
router.post('/publish', cookieParser(), express.json(), async(req, res) => {
    ...
    {
        const verify = jwt.verify(req.cookies['lvl2token'], JWT_KEY);
        const result = await publishPost(verify['id'], sanitizePost(req.body.post), req.body.public);
        res.json(result);
    }
    else
    {
