...
const JWT_KEY = 'lBTHdSVmCegf42djLlRLJC9NhbqFw938';
...
async function authenticateUser(username, password)
{
    ...
    try
    {
        ...
        console.log('Level 3: authenticateUser');
        console.log(err);
    }
}
...
async function createUser(username, name, password)
{
    ...
    {
        console.log('Level 3: createUser');
        console.log(err);
        return {
            status: 'failed',
...
{
    const token = req.cookies['lvl3token'];
    if (!token)
    {
        return false;
    }
    ...
    {
        console.log('Level 3: isAdminAuthenticated');
        console.log(err);
        return false;
    }
}
...
async function isUserAuthenticated(req)
{
    ...
    {
        console.log('Level 3: isUserAuthenticated');
        console.log(err);
        return false;
    }
}
...
router.all('/logout', async(req, res) => {
    res.clearCookie('lvl3token', { path: '/level3' });
    res.redirect('./');
});
...
router.post('/userinfo', cookieParser(), async(req, res) => {
    if (await isUserAuthenticated(req))
    {
        const verify = jwt.verify(req.cookies['lvl3token'], JWT_KEY);
        res.json(verify);
    }
    else
    {
...
async function publishPost(userID, postData, isPublic)
{
    ...
    {
        console.log('Level 3: publishPost');
        console.log(err);
        return {
            status: 'failed',
            message: err.toString()
